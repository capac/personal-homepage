...
A summary of the data preparation and feature engineering that are performed in the analysis are compiled in the bullet list below:
 ...
 - `weight`, `medical_specialty` and `payer_code` columns are removed due to the large presence of null values.
 ...
 - All variations of `Expired at...` or `Expired` in `discharge_disposition_id` are removed since they won't be responsible for further readmission cases. Null, not admitted and unknown/invalid values are removed as well.
